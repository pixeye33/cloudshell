...
wget -qO- $(curl -sL https://api.github.com/repos/junegunn/fzf/releases/latest | jq -r '.assets[] | select(.name | test("^fzf-.*-linux_amd64.tar.gz$")) | .browser_download_url') | tar -zxvf - fzf && install -c -m 0755 fzf ~/.local/bin/fzf

az extension add --name fzf

wget -qO- $(curl -sL https://api.github.com/repos/kubecolor/kubecolor/releases/latest | jq -r '.assets[] | select(.name | test("^kubecolor_.*_linux_amd64.tar.gz$")) | .browser_download_url') | tar -zxvf - kubecolor && install -c -m 0755 kubecolor ~/.local/bin/_kubecolor
